...
float strobe_freq_hz = 69.0;
uint32_t strobe_period_us = (uint32_t) 1e6/strobe_freq_hz;
uint32_t strobe_timer_count = 0;
int32_t  offset_us =0;
double offset = 0;
double modulation_time = 0;
double modulation_frequency_hz = 0.2;
...
void SchedulePulseCallback(){
  ...
  offset =10000;//(strobe_period_us-255.0*4)/2.0;
  ...
  train_b.AddPulse( (offset-strobe_period_us*0.125*sin(TWO_PI*modulation_time/modulation_period_us))
                          /tick_period_us,255.0*4/tick_period_us);
}
...
void loop() {
  ...
  if (panel.button.fell() ){
    ...
    if (update_freq){
      ...
      strobe_period_us=(uint32_t) 1e6*1.0/fan.fundamental_freq_hz;
      PulsePeriodTimer.setPeriod(  strobe_period_us   );
      update_freq = false;
      PulsePeriodTimer.start();
    }
